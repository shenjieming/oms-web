        ...
        link: function ($scope, element, attrs) {
            ...
            $scope.AreaController = {
                ...
                GetProvince: function () {
                    ...
                    $MessagService.loading("地区获取中，请稍等...");
                    $Api.BasisService.GetadmdivisionList({ level: 1 }, function (rData) {
                        $scope.AreaController.Province = rData.rows;
                    });
                },
                GetCity: function (code) {
                    /// <summary>获取市信息</summary>
                    $scope.ngModel.deliveryCityCode = Original.IsNewModel ? $scope.ngModel.deliveryCityCode : "";
                    if (code) {
                        $MessagService.loading("地区获取中，请稍等...");
                        $Api.BasisService.GetadmdivisionList({ level: 2, parentDivCode: code }, function (rData) {
                            $scope.AreaController.City = rData.rows;
                        });
                    } else { $scope.AreaController.City = new Array(); }
                },
                GetTownship: function (code) {
                    /// <summary>获取区信息</summary>
                    $scope.ngModel.deliveryDistrictCode = Original.IsNewModel ? $scope.ngModel.deliveryDistrictCode : "";
                    if (code) {
                        $MessagService.loading("地区获取中，请稍等...");
                        $Api.BasisService.GetadmdivisionList({ level: 3, parentDivCode: code }, function (rData) {
                            $scope.AreaController.Township = rData.rows;
                            if (Original.IsNewModel) {
                                setTimeout(function () {
                                    Original.IsNewModel = false;
                                },500);
            ...
            $scope.$watch("ngModel.deliveryProvinceCode", function () {
                ...
                if (!Original.IsNewModel) {
                    if (!$scope.ngModel.deliveryProvinceCode && $scope.ngModel.deliveryProvinceCode != "") {
                        $scope.AreaController.GetProvince();
                    }
                    $scope.AreaController.GetCity($scope.ngModel.deliveryProvinceCode);
                }
